...
import { Shield, Search, Filter, ChevronLeft, ChevronRight, User, Calendar } from "lucide-react";
    ...
    <AppLayout>
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Track who changed what and when across your organisation</p>
        </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b-2 border-black bg-[#F0F6FF]">
                      <th className="text-left px-4 py-3 font-black text-xs uppercase tracking-wider">Timestamp</th>
                      <th className="text-left px-4 py-3 font-black text-xs uppercase tracking-wider">User</th>
                      <th className="text-left px-4 py-3 font-black text-xs uppercase tracking-wider">Action</th>
                      <th className="text-left px-4 py-3 font-black text-xs uppercase tracking-wider">Entity</th>
                      <th className="text-left px-4 py-3 font-black text-xs uppercase tracking-wider">IP</th>
                      <th className="text-left px-4 py-3 font-black text-xs uppercase tracking-wider">Details</th>
                    </tr>
                  </thead>
                  <tbody>
                    {logs.map((log, i) => (
                      <>
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                <div className="font-bold text-black text-xs">{log.user?.name || log.userName || "System"}</div>
                                <div className="text-xs text-gray-400">{log.user?.email || log.userEmail || ""}</div>
                              </div>
                            </div>
                          </td>
                          <td className="px-4 py-3">
                            <span className={`px-2 py-0.5 text-xs font-black border rounded-sm whitespace-nowrap ${ACTION_COLORS[log.action] || "bg-gray-100 text-gray-700 border-gray-300"}`}>
                        ...
                        {expanded === log._id && log.details && Object.keys(log.details).length > 0 && (
                          <tr key={`${log._id}-detail`} className="bg-[#F0F6FF] border-b border-gray-200">
                            <td colSpan={6} className="px-6 py-3">
                              <div className="flex flex-wrap gap-3">
                                {Object.entries(log.details).map(([k, v]) => (
                                  <div key={k} className="bg-white border border-gray-300 px-3 py-1.5 rounded-sm">
                                    <span className="text-xs text-gray-500 font-bold uppercase">{k}: </span>
                                    <span className="text-xs font-black text-black">{String(v)}</span>
                                  </div>
                                ))}
                              </div>
                            </td>
