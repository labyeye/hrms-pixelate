...
export default function TasksPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.title.trim() || !form.assignedTo) {
      toast({
        title: "Title and assignee are required",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: err.message || "Failed to save task",
        variant: "destructive",
      });
    } finally {
      setSubmitting(false);
  ...
  const timelineGroups = useMemo(() => {
    ...
    if (withoutDate.length)
      groups.push({ label: "No due date", tasks: withoutDate });
    return groups;
  }, [filtered]);
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {COLUMNS.map((col) => (
            <div
              key={col.key}
              className="border-2 border-black bg-[#F8FAFF] flex flex-col"
            >
              <div
          ...
          <table className="w-full text-sm min-w-[720px]">
            <thead>
              <tr className="border-b-2 border-black bg-[#F8FAFF]">
                <th className="text-left px-3 py-2 font-bold uppercase text-xs">
                  Title
                </th>
                <th className="text-left px-3 py-2 font-bold uppercase text-xs">
                  Assignee
          ...
          {filtered.map((task) => {
            ...
            return (
              ...
              >
                ...
                />
                <p className="font-bold text-sm text-black flex-1 truncate">
                  {task.title}
                </p>
                  ...
                  <div className="max-h-40 overflow-y-auto space-y-2.5 p-2 bg-[#F8FAFF] border border-black/5">
                    {selected.comments?.length ? (
                      selected.comments.map((c, i) => (
                        <div
                          key={i}
                          className="text-xs border-b border-black/5 pb-2 last:border-0 last:pb-0"
                        >
