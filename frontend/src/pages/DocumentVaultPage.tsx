...
export default function DocumentVaultPage() {
  ...
  const [form, setForm] = useState({ employeeId: "", name: "", docType: "id_proof" });
  ...
  useEffect(() => {
    load();
    if (!isEmployee) {
      employeeAPI.getAll({ status: "active" }).then((r) => {
      ...
      {loading ? (
        ...
        <div className="border-2 bg-white p-12 flex flex-col items-center justify-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {!isEmployee ? "Upload documents for employees" : "No documents uploaded yet"}
          </p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm min-w-[700px]">
            <thead>
              <tr className="border-b-2 border-black bg-[#024BAB]/5">
                {["Document Name", "Type", !isEmployee ? "Employee" : null, "Size", "Uploaded", ""].filter(Boolean).map((h) => (
                  <th key={h as string} className="px-4 py-3 text-xs font-bold text-black uppercase tracking-wider text-left">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {displayed.map((doc) => {
                const TypeIcon = DOC_TYPES.find((t) => t.value === doc.docType)?.icon || File;
                return (
                  <tr key={doc._id} className="border-b border-black/10 hover:bg-[#024BAB]/5">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <TypeIcon className="w-4 h-4 text-[#024BAB] shrink-0" />
                        <span className="font-semibold text-black">{doc.name}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    {!isEmployee && (
                      <td className="px-4 py-3 font-medium text-black text-xs">
                        ...
                        <p className="text-[10px] text-muted-foreground">{doc.employee?.employeeId}</p>
                      </td>
                    )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="border-2 bg-white w-full max-w-md">
            <div className="flex items-center justify-between p-5 border-b-2 border-black">
              ...
              <button onClick={() => { setUploadModal(false); setFileObj(null); setForm({ employeeId: "", name: "", docType: "id_proof" }); }}>
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-5 space-y-4">
              <div>
                <label className="block text-xs font-bold text-black uppercase tracking-wider mb-1">Employee *</label>
                <select
              ...
              <div>
                <label className="block text-xs font-bold text-black uppercase tracking-wider mb-1">Document Type *</label>
                <select
                  value={form.docType}
              ...
              <div>
                <label className="block text-xs font-bold text-black uppercase tracking-wider mb-1">Document Name *</label>
                <input
                  type="text"
                  value={form.name}
              ...
              <div>
                <label className="block text-xs font-bold text-black uppercase tracking-wider mb-1">File * (max 4 MB)</label>
                <input ref={fileRef} type="file" onChange={handleFileChange} className="hidden" accept=".pdf,.jpg,.jpeg,.png,.doc,.docx" />
                <button
                  ...
                  className={cn(
                    ...
                    fileObj ? "border-[#00C48C] text-[#00C48C]" : "text-muted-foreground",
                  )}
                >
                  <Upload className="w-5 h-5" />
                  {fileObj ? fileObj.name : "Click to select file"}
                  {fileObj && <span className="text-[10px]">{formatBytes(fileObj.size)}</span>}
                </button>
              </div>
              <button
                onClick={handleUpload}
                disabled={uploading || !fileObj || !form.name || !form.employeeId}
