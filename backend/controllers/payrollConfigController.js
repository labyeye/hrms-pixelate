...
const upsertDeductionRules = asyncHandler(async (req, res) => {
  const {
    ...
    lateThresholdMinutes, lateDeductionType, lateDeductionAmount,
    halfDayThresholdMinutes, halfDayDeductionPercent,
    absentDeductionType, absentDeductionAmount,
    earlyCheckoutThresholdMinutes, earlyCheckoutDeductionEnabled,
    workWeek,
    enablePf, pfRate,
    enableEsi, esiRate, esiGrossLimit,
    enableTds, tdsRate, tdsTaxableThreshold,
    defaultHraPct, defaultDaPct, defaultTa,
  } = req.body;
  ...
  const rule = await DeductionRule.findOneAndUpdate(
    { company: req.user.company },
    {
      $set: {
        ...
        halfDayDeductionPercent: Number(halfDayDeductionPercent) ?? 50,
        absentDeductionType: absentDeductionType || "percent",
        absentDeductionAmount: Number(absentDeductionAmount) ?? 100,
        earlyCheckoutThresholdMinutes: Number(earlyCheckoutThresholdMinutes) ?? 15,
        earlyCheckoutDeductionEnabled: Boolean(earlyCheckoutDeductionEnabled),
        workWeek: workWeek || "mon_sat",
        enablePf: enablePf !== false,
        pfRate: Number(pfRate) ?? 12,
        enableEsi: enableEsi !== false,
        esiRate: Number(esiRate) ?? 1.75,
        esiGrossLimit: Number(esiGrossLimit) ?? 21000,
        enableTds: Boolean(enableTds),
        tdsRate: Number(tdsRate) ?? 10,
        tdsTaxableThreshold: Number(tdsTaxableThreshold) ?? 50000,
        defaultHraPct: Number(defaultHraPct) ?? 40,
        defaultDaPct: Number(defaultDaPct) ?? 10,
        defaultTa: Number(defaultTa) ?? 1500,
      },
    },
    { upsert: true, new: true },
  );
