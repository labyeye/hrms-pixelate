...
const ADMIN_STATS = [
  { label: 'Total Employees', key: 'totalEmployees', icon: Users, color: C.primary },
  { label: 'Present Today', key: 'todayPresent', icon: CheckCircle2, color: C.success },
  { label: 'On Leave', key: 'todayOnLeave', icon: CalendarOff, color: C.warning },
  { label: 'Pending Leaves', key: 'pendingLeaves', icon: Clock, color: C.danger },
  { label: 'Departments', key: 'departments', icon: Building2, color: C.primary },
  { label: 'Open Positions', key: 'openPositions', icon: Briefcase, color: C.secondary },
];
...
function AdminDashboard({ navigation }: any) {
  ...
  const handleCardPress = (key: string) => {
    ...
    else if (key === 'todayOnLeave' || key === 'pendingLeaves') navigation.navigate('Leave');
    else if (key === 'departments') navigation.navigate('More', { screen: 'Departments' });
  };
        ...
        <View style={styles.headerAvatarWrap}>
          {user?.avatar ? (
            ...
            <View style={[styles.headerAvatar, styles.headerAvatarFallback]}>
              <Text style={styles.headerAvatarText}>{(user?.name || 'U')[0].toUpperCase()}</Text>
            </View>
          )}
        </View>
        <View style={{ flex: 1 }}>
        ...
        >
          {error && (
            <View style={styles.errorBanner}>
              ...
              <TouchableOpacity onPress={load}><RefreshCw size={14} color={C.danger} /></TouchableOpacity>
            </View>
          )}
          ...
          <View style={styles.grid}>
            {ADMIN_STATS.map(s => {
              const Icon = s.icon;
              return (
                <TouchableOpacity key={s.key} style={styles.statCard} activeOpacity={0.8} onPress={() => handleCardPress(s.key)}>
          ...
          <View style={styles.card}>
            {[
              { label: 'Payroll Processed', val: stats?.monthlyPayroll ?? '—', icon: <TrendingUp size={14} color={C.success} /> },
              { label: 'New Hires', val: stats?.newHires ?? '—', icon: <Users size={14} color={C.primary} /> },
              { label: 'Leave Requests', val: stats?.pendingLeaves ?? '—', icon: <Clock size={14} color={C.warning} /> },
            ].map((item, i) => (
              <View key={item.label} style={[styles.quickRow, i > 0 && styles.quickBorder]}>
...
function EmployeeDashboard({ navigation }: any) {
  ...
  // Month stats
  const presentDays = myAttendance.filter(a =>
    ['present', 'late', 'half_day'].includes(a.status) || a.checkIn,
  ).length;
        ...
        <View style={{ flex: 1 }}>
          ...
          {(empProfile?.designation || empProfile?.department?.name) ? (
            <Text style={styles.company}>
              {[empProfile?.designation, empProfile?.department?.name].filter(Boolean).join(' · ')}
            </Text>
          ) : null}
        </View>
        <TouchableOpacity onPress={() => navigation.navigate('More', { screen: 'Notifications' })} style={styles.bellBtn}>
        ...
        >
          ...
          <View style={styles.profileBanner}>
            <View style={styles.profileBannerLeft}>
              {user?.avatar ? (
                <Image source={{ uri: user.avatar }} style={styles.profileBannerAvatar} />
              ) : (
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.profileBannerRole}>
                  {[empProfile?.designation, empProfile?.department?.name].filter(Boolean).join(' · ') || user?.role?.replace(/_/g, ' ') || 'Employee'}
                </Text>
            ...
            <View style={styles.profileBannerStats}>
              ...
              <View style={[styles.profileBannerStat, { borderLeftWidth: 1, borderLeftColor: '#E5E7EB' }]}>
                ...
                <Text style={styles.profileBannerStatVal}>
                  {empProfile?.joinDate
                    ? new Date(empProfile.joinDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })
                    : '—'}
                </Text>
          ...
          {/* Today's status */}
          <Text style={styles.sectionLabel}>{`Today — ${now.toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long' })}`}</Text>
          <View style={styles.todayCard}>
            {todayRecord ? (
              <View style={styles.todayRow}>
                <View style={[styles.todayStatusBadge, { backgroundColor: STATUS_COLOR[todayRecord.status] || C.textMuted }]}>
                  <Text style={styles.todayStatusText}>{todayRecord.status.replace('_', ' ').toUpperCase()}</Text>
                </View>
                <View style={styles.todayTimes}>
                  <View style={styles.todayTimeItem}>
                    ...
                    <Text style={styles.todayTimeVal}>{todayRecord.checkIn ? fmtTime(todayRecord.checkIn) : '—'}</Text>
                  </View>
                  <View style={styles.todayTimeItem}>
                    <LogIn size={13} color={C.danger} style={{ transform: [{ scaleX: -1 }] }} />
                    <Text style={styles.todayTimeLabel}>Check Out</Text>
                    <Text style={styles.todayTimeVal}>{todayRecord.checkOut ? fmtTime(todayRecord.checkOut) : '—'}</Text>
                  </View>
                  {todayRecord.workHours > 0 && (
                    <View style={styles.todayTimeItem}>
                      <Clock size={13} color={C.primary} />
                      <Text style={styles.todayTimeLabel}>Hours</Text>
                      <Text style={styles.todayTimeVal}>{todayRecord.workHours.toFixed(2)}h</Text>
                    </View>
                  )}
                </View>
              </View>
              ...
              <View style={styles.todayAbsent}>
                ...
                <Text style={styles.todayAbsentText}>No attendance marked today</Text>
              </View>
            )}
          </View>
            ...
            <>
              ...
              >
                <View style={styles.payslipLeft}>
                  ...
                  <View>
                    <Text style={styles.payslipMonth}>{MONTHS[(latestPayroll.month || 1) - 1]} {latestPayroll.year}</Text>
                    <Text style={styles.payslipStatus}>{latestPayroll.status?.toUpperCase()}</Text>
                  </View>
                </View>
                <Text style={styles.payslipNet}>₹{(latestPayroll.netSalary || 0).toLocaleString()}</Text>
              </TouchableOpacity>
            </>
          )}
          ...
          {recent.length > 0 && (
            <>
              ...
              <View style={styles.card}>
                {recent.map((a, i) => (
                  <View key={a._id} style={[styles.attRow, i > 0 && styles.attBorder]}>
                    <Text style={styles.attDate}>
                      {new Date(a.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', weekday: 'short' })}
                    </Text>
                    <View style={[styles.attBadge, { backgroundColor: (STATUS_COLOR[a.status] || C.textMuted) + '22' }]}>
                      <Text style={[styles.attBadgeText, { color: STATUS_COLOR[a.status] || C.textMuted }]}>
                        {a.status.replace('_', ' ').toUpperCase()}
                      </Text>
                    </View>
                    <View style={styles.attTimes}>
                      <Text style={styles.attTime}>{a.checkIn ? fmtTime(a.checkIn) : '—'}</Text>
                      <Text style={[styles.attTime, { color: C.textMuted }]}>{a.checkOut ? fmtTime(a.checkOut) : '—'}</Text>
                    </View>
                  </View>
                ))}
              </View>
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    backgroundColor: C.white, borderBottomWidth: 2, borderBottomColor: C.black,
  },
  ...
  headerAvatar: { width: 44, height: 44, borderRadius: 22, borderWidth: 2, borderColor: C.black },
  headerAvatarFallback: { backgroundColor: C.primary, alignItems: 'center', justifyContent: 'center' },
  ...
  errorBanner: {
    ...
    backgroundColor: '#FEF2F2', borderWidth: 2, borderColor: C.danger, padding: 10, marginBottom: 16,
  },
  errorText: { fontSize: 12, color: C.danger, fontWeight: '700', flex: 1 },
  sectionLabel: {
    fontSize: 11, fontWeight: '700', textTransform: 'uppercase',
    color: C.textMuted, marginBottom: 10, marginTop: 16, letterSpacing: 0.5,
  },

  // Admin grid
  grid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginBottom: 8 },
  statCard: { width: CARD_WIDTH, borderWidth: 2, borderColor: C.black, backgroundColor: C.white, padding: 14 },
  statIconWrap: { width: 38, height: 38, alignItems: 'center', justifyContent: 'center', marginBottom: 10, borderWidth: 2, borderColor: C.black },
  statValue: { fontSize: 28, fontWeight: '700', color: C.black },
  statLabel: { fontSize: 11, fontWeight: '700', color: C.black, marginTop: 2 },

  // Shared card
  card: { backgroundColor: C.white, borderWidth: 2, borderColor: C.black, marginBottom: 4 },
  quickRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 10, gap: 10, paddingHorizontal: 16 },
  quickBorder: { borderTopWidth: 1, borderTopColor: '#F3F4F6' },
  quickIcon: { width: 28, height: 28, backgroundColor: '#F3F4F6', alignItems: 'center', justifyContent: 'center' },
  ...
  profileBannerAvatar: { width: 56, height: 56, borderRadius: 28, borderWidth: 2, borderColor: C.black },
  profileBannerAvatarFb: { backgroundColor: C.primary, alignItems: 'center', justifyContent: 'center' },
  ...
  profileBannerRole: { fontSize: 12, color: C.textMuted, fontWeight: '500', marginTop: 2 },
  profileBannerEmpId: { fontSize: 11, color: C.primary, fontWeight: '700', marginTop: 2 },
  profileBannerStats: { flexDirection: 'row' },
  profileBannerStat: { flex: 1, alignItems: 'center', paddingVertical: 12, gap: 3 },
  profileBannerStatVal: { fontSize: 13, fontWeight: '700', color: C.black },
  profileBannerStatLabel: { fontSize: 10, color: C.textMuted, fontWeight: '500', textTransform: 'uppercase' },

  // Employee: Today card
  todayCard: { backgroundColor: C.white, borderWidth: 2, borderColor: C.black, padding: 16, marginBottom: 4 },
  todayRow: { gap: 14 },
  todayStatusBadge: { alignSelf: 'flex-start', paddingHorizontal: 12, paddingVertical: 4 },
  todayStatusText: { color: C.white, fontSize: 11, fontWeight: '700', letterSpacing: 0.5 },
  todayTimes: { flexDirection: 'row', gap: 20 },
  todayTimeItem: { gap: 2, alignItems: 'center' },
  todayTimeLabel: { fontSize: 10, color: C.textMuted, fontWeight: '600', textTransform: 'uppercase' },
  ...
  monthStatCard: {
    flex: 1, backgroundColor: C.white, borderWidth: 2, borderColor: C.black,
    alignItems: 'center', paddingVertical: 14, gap: 6,
  },
  monthStatIcon: { width: 32, height: 32, alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: C.black },
  monthStatVal: { fontSize: 22, fontWeight: '700' },
  monthStatLabel: { fontSize: 10, fontWeight: '700', color: C.black, textTransform: 'uppercase' },
  ...
  payslipStatus: { fontSize: 10, fontWeight: '700', color: C.success, marginTop: 2 },
  payslipNet: { fontSize: 22, fontWeight: '700', color: C.black },

  // Employee: Recent attendance
  attRow: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 11, gap: 10 },
  ...
  quickActionBtn: {
    ...
    gap: 8, paddingVertical: 14, borderWidth: 2, borderColor: C.black,
  },
  quickActionText: { color: C.white, fontSize: 13, fontWeight: '700', textTransform: 'uppercase' },
});
