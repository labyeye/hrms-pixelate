...
export default function ForgotPasswordScreen({ navigation }: any) {
  ...
  const handleSubmit = async () => {
    ...
    if (!trimmed) { Alert.alert('Validation', 'Please enter your email address'); return; }
            ...
            {sent ? (
              <View style={styles.successBlock}>
                ...
                <Text style={styles.cardSub}>
                  If an account with <Text style={{ fontWeight: '700' }}>{email.trim()}</Text> exists,
...
const styles = StyleSheet.create({
  ...
  card: {
    backgroundColor: C.white, borderWidth: 2, borderColor: C.black, padding: 24,
  },
  cardTitle: { fontSize: 22, fontWeight: '700', color: C.black, marginBottom: 8 },
  cardSub: { fontSize: 13, color: '#6B7280', fontWeight: '500', marginBottom: 20, lineHeight: 20 },
  field: { marginBottom: 16 },
  label: {
    fontSize: 10, fontWeight: '700', textTransform: 'uppercase', color: C.black,
    marginBottom: 6, letterSpacing: 0.5,
  },
  inputRow: {
    flexDirection: 'row', alignItems: 'center',
    borderWidth: 2, borderColor: C.black, backgroundColor: C.white,
  },
  inputIcon: { marginHorizontal: 10 },
  input: { flex: 1, paddingVertical: 12, paddingRight: 12, fontSize: 14, fontWeight: '500', color: C.black },
  btnPrimary: {
  ...
  btnPrimaryText: { color: C.white, fontWeight: '700', fontSize: 14, textTransform: 'uppercase', letterSpacing: 0.5 },
  ...
  successIcon: {
    width: 64, height: 64, borderWidth: 2, borderColor: C.success,
